...
public class Config2dSceneWriter extends BaseWriter
{
	...
	private void writeSceneCfg(Scene scene) throws Exception
	{
		...
		for (SceneDoor door : scene.doors)
		{
			...
			if (hot != null)
			{
				...
				sb.append(String.format("\t\t\t<hot id=\"%s\" x=\"%s\" y=\"%s\" width=\"%s\" height=\"%s\" acceptableQuests=\"%s\" acceptedQuests=\"%s\" submitableQuests=\"%s\" submitedQuests=\"%s\">\n", hot.id, hot.x, hot.y, hot.width, hot.height, hot.acceptableQuests, hot.acceptedQuests, hot.submitableQuests, hot.submitedQuests));
	...
	private void writerWorldCfg() throws Exception
	{
		...
		// 存储文件
		byte[] bytes = sb.toString().getBytes("UTF-8");
		FileUtil.writeFile(new File(root.getOutputFolder().getPath() + "/$World.2d.xml"), bytes);
